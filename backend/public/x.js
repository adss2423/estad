// Obtener el lienzo donde se dibujara la gráfica
        const ctxF = document.getElementById('graficaX');
        ...
        async function cargarDatosX() {
            try {
                const respuesta = await fetch('/api/estadisticas/x');
                ...
                // Dibujamos la gráfica con los datos reales
                graficaActualF = new Chart(ctxF, {
                    type: 'line',
                    data: {
                        labels: etiquetasFechas, // Las fechas que vienen de MySQL
                        datasets: [{
                            label: 'Seguidores de X',
                            data: valoresSeguidores, // Los números que vienen de MySQL
                            borderColor: isDarkTheme ? '#d4d4d4' : '#2b2b2b',
                            tension: 0.3,
                            fill: true,
                            backgroundColor: isDarkTheme ? 'rgba(212, 212, 212, 0.14)' : 'rgba(123, 123, 123, 0.1)'
                        }]
                    },
                    options: { 
